import { createDeck, shuffle, newGame, playCard, drawToHand, laySequence, checkWin, Card, cardColor } from '../src/shared/game/core';
...
describe('Game core basic behaviors', () => {
  ...
  test('playing Ace causes player to draw two extra', () => {
    ...
    const res = playCard(state, 0, idx);
  ...
  test('laying a valid sequence removes cards from hand and adds to sequences', () => {
    const state = newGame(() => 0.123);
    // craft a player with a known sequence 2,3,4 Clubs (black)
    const c2: Card = { id: 'C-2', suit: 'Clubs', rank: '2' };
    const c3: Card = { id: 'C-3', suit: 'Clubs', rank: '3' };
    const c4: Card = { id: 'C-4', suit: 'Clubs', rank: '4' };
    state.players[0].hand = [c2, c3, c4];
    const res = laySequence(state, 0, [0,1,2]);
    expect(res.ok).toBe(true);
  ...
  test('playing Joker with delete removes enemy sequence and moves its cards to discard', () => {
    const state = newGame(() => 0.7);
    // create a sequence for player 1 (enemy)
    const s1: Card = { id: 's1', suit: 'Hearts', rank: '2' };
    const s2: Card = { id: 's2', suit: 'Hearts', rank: '3' };
    const s3: Card = { id: 's3', suit: 'Hearts', rank: '4' };
    state.players[1].sequences.push([s1, s2, s3]);

    // give player 0 a Joker in hand
    const joker: Card = { id: 'J-TEST', suit: 'Joker', rank: 'JOKER' };
    ...
    const res = playCard(state, 0, idx, { action: 'delete', seqIndex: 0 });
    expect(res.ok).toBe(true);
  ...
  test('cannot lay a sequence composed only of Jokers', () => {
    ...
    const j1: Card = { id: 'J1', suit: 'Joker', rank: 'JOKER' };
    const j2: Card = { id: 'J2', suit: 'Joker', rank: 'JOKER' };
    state.players[0].hand = [j1, j2];
    const res = laySequence(state, 0, [0, 1]);
    expect(res.ok).toBe(false);
  });

  test('mixed Jokers and cards can form a valid sequence (joker fills the gap)', () => {
    const state = newGame(() => 0.3);
    // 2, Joker, 4 of Hearts -> should be treated as 2,3,4 Hearts
    const c2: Card = { id: 'H-2', suit: 'Hearts', rank: '2' };
    const joker: Card = { id: 'J-MID', suit: 'Joker', rank: 'JOKER' };
    const c4: Card = { id: 'H-4', suit: 'Hearts', rank: '4' };
  ...
  test('sequence with jokers but non-matching non-joker colors is rejected', () => {
    ...
    const c2: Card = { id: 'H-2-bad', suit: 'Hearts', rank: '2' };
    const joker: Card = { id: 'J-BAD', suit: 'Joker', rank: 'JOKER' };
    const c4: Card = { id: 'C-4-bad', suit: 'Clubs', rank: '4' };
    state.players[0].hand = [c2, joker, c4];
    const res = laySequence(state, 0, [0, 1, 2]);
    expect(res.ok).toBe(false);
  });
  ...
  test('Ace can be used as 1 in a sequence (A,2,3)', () => {
    const state = newGame(() => 0.6);
    const a: Card = { id: 'HA', suit: 'Hearts', rank: 'A' };
    const two: Card = { id: 'H2', suit: 'Hearts', rank: '2' };
    const three: Card = { id: 'H3', suit: 'Hearts', rank: '3' };
  ...
  test('Jack/Queen/King numeric values work in sequences (10,J,Q and J,Q,K)', () => {
    ...
    const ten: Card = { id: 'H10', suit: 'Hearts', rank: '10' };
    const jack: Card = { id: 'HJ', suit: 'Hearts', rank: 'J' };
    const queen: Card = { id: 'HQ', suit: 'Hearts', rank: 'Q' };
    const king: Card = { id: 'HK', suit: 'Hearts', rank: 'K' };

    // 10, J, Q => 10,11,12
    state.players[0].hand = [ten, jack, queen];
    let res = laySequence(state, 0, [0,1,2]);
    expect(res.ok).toBe(true);
    expect(state.players[0].sequences.length).toBe(1);

    // reset for J,Q,K => 11,12,13
    state.players[0].hand = [jack, queen, king];
    res = laySequence(state, 0, [0,1,2]);
    expect(res.ok).toBe(true);
