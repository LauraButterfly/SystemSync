...
const core_1 = require("../shared/game/core");
...
function fullStatePayload(room) {
    return {
        state: room.state,
        meta: {
...
io.on('connection', socket => {
    ...
    socket.on('createRoom', (modeOrCb, cb) => {
        ...
        if (typeof modeOrCb === 'function')
            cb = modeOrCb;
        const id = makeId();
        const room = { id, sockets: [socket.id], players: {} };
    ...
    socket.on('joinRoom', (roomId, cb) => {
        ...
        if (room.sockets.length >= 2)
            return cb({ ok: false, reason: 'Room full' });
    ...
    socket.on('startGame', (roomId, cb) => {
        const room = rooms.get(roomId);
        if (!room)
            return cb && cb({ ok: false, reason: 'Room not found' });
        // only host (playerIndex 0) may start
        const idx = room.players[socket.id];
        if (idx !== 0)
            return cb && cb({ ok: false, reason: 'Only the Host can start the game' });
        ...
        if (playerCount < 2)
            return cb && cb({ ok: false, reason: 'Need 2 players to start' });
        // start game with smaller hands in sudden-death mode
        const startingHandSize = room.mode === 'sudden-death' ? 3 : 8;
        room.state = (0, core_1.newGame)(undefined, startingHandSize);
        ...
        (0, core_1.drawToHand)(room.state, starter, 1);
        // auto top-up to four if below four after the mandatory draw
        (0, core_1.ensureHandSizeFour)(room.state, starter);
    ...
    socket.on('playCard', (data, cb) => {
        ...
        if (!room || !room.state)
            return cb && cb({ ok: false, reason: 'No game' });
        // enforce one discard per turn: if this player already discarded, block further plays
        if (room.discardedThisTurnFor === data.playerIndex)
            return cb && cb({ ok: false, reason: 'Already discarded this turn' });
        ...
        const res = (0, core_1.playCard)(room.state, data.playerIndex, data.handIndex, data.options);
        if (!res.ok)
            return cb && cb(res);
        ...
        if (top && top.rank === 'J') {
            ...
            if (addedIds.length > 0) {
                ...
                try {
                    ...
                    const thiefHand = sanitized.players[data.playerIndex].hand;
        ...
        const win = require('../shared/game/core').checkWin(room.state, room.sequencesToWin ?? 3);
        if (win && win.winner !== undefined)
            io.to(data.roomId).emit('gameOver', win);
        cb && cb({ ok: true });
    });
    socket.on('endTurn', (roomId, cb) => {
        ...
        if (room.state.currentPlayer !== playerIdx)
            return cb && cb({ ok: false, reason: 'Not your turn' });
        // player must discard one card before ending the turn
        if (room.discardedThisTurnFor !== playerIdx)
            return cb && cb({ ok: false, reason: 'You must discard one card before ending your turn' });
        ...
        if (extra > 0) {
            ...
            room.drawnThisTurnCount = 0;
            room.discardedThisTurnFor = null;
            (0, core_1.drawToHand)(room.state, playerIdx, 1);
            // auto top-up to four after mandatory draw for new turn
            (0, core_1.ensureHandSizeFour)(room.state, playerIdx);
            room.drawnThisTurnCount = 1;
            io.to(roomId).emit('stateUpdate', fullStatePayload(room));
        ...
        if (room.state.players[next].blockedRounds > 0) {
            ...
            (0, core_1.drawToHand)(room.state, skipTo, 1);
            // auto top-up to four after mandatory draw
            (0, core_1.ensureHandSizeFour)(room.state, skipTo);
            room.drawnThisTurnCount = 1;
        }
        else {
            ...
            room.discardedThisTurnFor = null;
            // mandatory draw for new turn
            (0, core_1.drawToHand)(room.state, next, 1);
            // auto top-up to four after mandatory draw
            (0, core_1.ensureHandSizeFour)(room.state, next);
            room.drawnThisTurnCount = 1;
        }
        io.to(roomId).emit('stateUpdate', fullStatePayload(room));
        cb && cb({ ok: true });
    });
    socket.on('laySequence', (data, cb) => {
        const room = rooms.get(data.roomId);
        if (!room || !room.state)
            return cb && cb({ ok: false, reason: 'No game' });
        // player must have discarded this turn before laying sequences
        if (room.discardedThisTurnFor !== data.playerIndex)
            return cb && cb({ ok: false, reason: 'You must discard one card before laying sequences' });
        const res = (0, core_1.laySequence)(room.state, data.playerIndex, data.handIndices);
        if (!res.ok)
            return cb && cb(res);
        io.to(data.roomId).emit('stateUpdate', fullStatePayload(room));
        const win = require('../shared/game/core').checkWin(room.state, room.sequencesToWin ?? 3);
    ...
    socket.on('getState', (roomId, cb) => {
        const room = rooms.get(roomId);
        if (!room || !room.state)
            return cb({ ok: false, reason: 'No game' });
        cb({ ok: true, state: room.state });
    });
    socket.on('reorderTop', (data, cb) => {
        const room = rooms.get(data.roomId);
        if (!room || !room.state)
            return cb && cb({ ok: false, reason: 'No game' });
        if (!room.pendingReorderBy || room.pendingReorderBy !== socket.id)
            return cb && cb({ ok: false, reason: 'Not authorized to reorder' });
        // apply reorder
        try {
            const { reorderTopThree } = require('../shared/game/core');
    ...
    socket.on('sortHand', (roomId, cb) => {
        ...
        if (playerIdx === undefined)
            return cb && cb({ ok: false, reason: 'Not in room' });
        try {
            const { sortHand } = require('../shared/game/core');
    ...
    socket.on('drawUpToFour', (roomId, cb) => {
        const room = rooms.get(roomId);
        if (!room || !room.state)
            return cb && cb({ ok: false, reason: 'No game' });
        const playerIdx = room.players[socket.id];
        ...
        if ((room.drawnThisTurnCount ?? 0) >= 2)
            return cb && cb({ ok: false, reason: 'Already drawn (top-up) this turn' });
        ...
        io.to(roomId).emit('stateUpdate', fullStatePayload(room));
        if (drawn > 0)
            io.to(roomId).emit('cardsDrawn', { playerIndex: playerIdx, count: drawn, newHandSize: after });
        cb && cb({ ok: true, drawn });
    });
